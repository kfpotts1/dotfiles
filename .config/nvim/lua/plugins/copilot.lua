...
return {
  -- NOTE: kenny: currently disabled the copilot.lua plugin in favor of the
  -- original for now until I can get all of the features working as I would
  -- like. I like the panel feature, but keymaps are not working as expected.
  --
  --[[
  {
    ...
    config = function()
      ...
      -- example keymap: next = '<M-[>', previous = '<M-]>'
    end,
  },
  --]]
  {
    ...
    config = function()
      ...
      -- NOTE: kenny: disabled in favor of codeium and lua copilot.
      -- may go back to this later. Trying out the others.
  ...
  },
  {
    ...
    opts = function(_, opts)
      -- Retrieve the existing spec table
      local existing_spec = opts.spec or {}

      -- Define the new Copilot-specific mappings
      local copilot_spec = {
      ...
      -- Merge the existing spec with the new Copilot-specific mappings
      for _, mapping in ipairs(copilot_spec) do
        table.insert(existing_spec, mapping)
      end

      -- Assign the merged spec table back to opts.spec
      opts.spec = existing_spec
    end,
  },
  {
    ...
    dependencies = {
      ...
      -- { 'zbirenbaum/copilot.lua' }, -- for the Copilot plugin
    ...
    config = function(_, opts)
      ...
      -- NOTE: @kenny: method deprecated, see new definitions above.
